...
class BackBoneConfig:
    ...
    num_layers: int = 2  # originally 30
...
class NgramHashMapping:
    def __init__(
        ...
        print("Tokenizer vocab size:", self.tokenizer_vocab_size)
...
class MultiHeadEmbedding(nn.Module):
    def __init__(self, list_of_N: List[int], D: int):
        ...
        print("Embedding table size (GB):", total_N * D * 4 / 1024 / 1024 / 1024)

        # 把所有表串联成一个大表 + offsets 映射
        # 各 head 各占一块区间，互不重叠
        self.embedding = nn.Embedding(num_embeddings=total_N, embedding_dim=D)

    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        shifted_input_ids = (
            input_ids + self.offsets
        )  # [B, T, n_head_per_ngram * (max_ngram_size-1)]
