...
__global__ void kernel_4(half* A, half* B, half* C, half* D, const float alpha, const float beta, const unsigned int M,
  ...
  /*
    ...
    TILE_COLS = BK_dim = 128
    TILE_COLS_VECTORIZED = 128/8 = 16（float4 粒度）
    ROW_STEP = NUM_THREADS/TILE_COLS_VECTORIZED = 256/16 = 16
    NUM_ITERS = 256/16 = 16

    B tile:
    TILE_ROWS = BK_dim = 128
    TILE_COLS = BN_dim = 128
    TILE_COLS_VECTORIZED = 128/8 = 16（float4 粒度）
    ROW_STEP = NUM_THREADS/TILE_COLS_VECTORIZED = 256/16 = 16
    NUM_ITERS = TILE_ROWS/ROW_STEP = 128/16 = 8
  */
  float4 A_gmem_cache_reg[16];  // tileMemcpyLoad 每个线程缓存 16 个 float4
  float4 B_gmem_cache_reg[8];   // tileMemcpyLoad 每个线程缓存 8 个 float4
  ...
  for (unsigned int block_k = 1; block_k <= num_block_tiles_k; block_k++) {
    ...
    // preload next block tile from global memory into per-thread register cache.
    if (block_k != num_block_tiles_k) {
      ...
      tileMemcpyLoad<BM_dim, BK_dim, NUM_THREADS, 16>(A_block_gmem, A_gmem_cache_reg, K);
      tileMemcpyLoad<BK_dim, BN_dim, NUM_THREADS, 8>(B_block_gmem, B_gmem_cache_reg, N);
    }

    // compute current block tile from shared memory.
    ...
    // prefetch next tile from registers into shared memory.
    if (block_k != num_block_tiles_k) {
      tileMemcpySwizzleStore<BM_dim, BK_dim, NUM_THREADS, SWIZZLE_BITS_A, 16>(A_gmem_cache_reg, A_block_smem);
      tileMemcpySwizzleStore<BK_dim, BN_dim, NUM_THREADS, SWIZZLE_BITS_B, 8>(B_gmem_cache_reg, B_block_smem);
    }
  }
...
void kernel_4_launch(sgemm_params device_sgemm_params, KernelLogger& timer, const unsigned int num_runs = 10) {
  ...
  for (int i = 0; i < num_runs; i++) {
    timer.Start();
    kernel_4<BM_dim, BN_dim, BK_dim, WM_dim, WN_dim, WK_dim, NumThreads><<<gridDim, blockDim, shmem_bytes>>>(
