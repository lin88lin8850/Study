...
__global__ void kernel_6(half* A, half* B, half* C, half* D, const float alpha, const float beta, const unsigned int M,
...
void kernel_6_launch(sgemm_params device_sgemm_params, KernelLogger& timer, const unsigned int num_runs = 10) {
  ...
  for (int i = 0; i < num_runs; i++) {
    timer.Start();
    kernel_6<BM_dim, BN_dim, BK_dim, WM_dim, WN_dim, WK_dim, NumThreads><<<gridDim, blockDim, shmem_bytes>>>(
