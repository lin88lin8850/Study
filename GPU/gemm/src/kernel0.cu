 ...
 */
__global__ void kernel_0(half* A, half* B, half* C, half* D, const float alpha, const float beta, const unsigned int M,
  ...
  // 写回结果
  D[m * N + n] = __float2half(alpha * acc + (beta * __half2float(C[m * N + n])));
}

/**
 * kernel_0_launch: 启动 kernel_0
 */
void kernel_0_launch(sgemm_params device_sgemm_params, KernelLogger& timer, const unsigned int num_runs = 10) {
  ...
  for (int i = 0; i < num_runs; i++) {
    timer.Start();
    kernel_0<<<gridDim, blockDim>>>(device_sgemm_params.A, device_sgemm_params.B, device_sgemm_params.C,
                                    device_sgemm_params.D, device_sgemm_params.alpha, device_sgemm_params.beta, M, N,
                                    K);
    timer.Stop();
  }
