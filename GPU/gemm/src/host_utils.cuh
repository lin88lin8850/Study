...
std::pair<sgemm_params, sgemm_params> sgemm_setup(unsigned int M,
                                                  unsigned int N,
                                                  unsigned int K,
                                                  float alpha = 0.7,
                                                  float beta = 0.3) {
  ...
  sgemm_params device_sgemm_params = {dev_A, dev_B, dev_C, dev_D, alpha,
                                      beta,  M,     N,     K};
...
bool elementwise_isclose(half* a, half* b, int size, float atol = 0.5) {
  for (int i = 0; i < size; i++) {
    if (std::abs((float)a[i] - (float)b[i]) > atol) {
      std::cout << "Mismatch at index " << i << ": " << (float)a[i]
                << " != " << (float)b[i] << std::endl;
      return false;
    }
  }
...
void sgemm_verify(sgemm_params device_sgemm_params,
                  sgemm_params host_sgemm_params) {
  const unsigned int M = host_sgemm_params.M;
  const unsigned int N = host_sgemm_params.N;
  half* D = (half*)malloc(M * N * sizeof(half));
  // Ensure any prior kernels have completed and surface device errors early
  CUDA_CHECK(cudaDeviceSynchronize());

  // Copy device result back to host for verification
  CUDA_CHECK(cudaMemcpy(D, device_sgemm_params.D, M * N * sizeof(half),
                        cudaMemcpyDeviceToHost));

  // Compute reference result on host
  host_sgemm(host_sgemm_params);

  // Compare results and handle mismatch gracefully (avoid assert which can
  // abort unsafely)
  if (!elementwise_isclose(D, host_sgemm_params.D, M * N)) {
    std::cerr << "SGEMM verification FAILED (see mismatch above)\n";
    // Exit after syncing to avoid leaving other device work in-flight
    exit(EXIT_FAILURE);
  }
}
