...
/*
  [WM_dim, WK_dim] = [128, 32]
  ldmatrix_a 拿的是 A 的 “32 行 x 8 列” 片段, 可理解为 4 个 8x8 沿 M 方向堆叠, m32n8

  1). WM_dim=128, 128/m32=4, 4 个 m8n8.x4 ldmatrix 指令走完 M 方向, 每次地址偏移 m32 行
  2). WK_dim=32, 32/n8=4, 4 个 m8n8.x4 ldmatrix 指令走完 K 方向
      warp 在 K 方向切分了 4 个子块 {k0, k1, k2, k3}, 每个子块在 K 方向跨 n8*2=16B
      由于有 4 块，K 方向块之间的地址偏移，需要条件翻转 bit5b4
      k0: 假设初始 src_addr 的 b5b4=00
      k1: src_addr^=0001_0000 后, b5b4=01
      k2: src_addr^=0011_0000 后, b5b4=10
      k4: src_addr^=0001_0000 后, b5b4=11
*/
...
/*
  [WK_dim, WN_dim] = [32, 64]
  ldmatrix_b（trans）拿的是 B 的 “8 行 x 32 列” 片段, 可理解为 4 个 8x8 沿 N 方向并排, m8n32

  1). WK_dim=32, 32/m8=4, 4 个 m8n8.x4 ldmatrix 指令走完 K 方向, 每次地址偏移 m8 行
  2). WN_dim=64, 64/n32=2, 2 个 m8n8.x4 ldmatrix 指令走完 N 方向
      warp 在 N 方向切分了 2 个子块 {n0, n1}, 每个子块在 N 方向跨 n32*2=64B,跨 64B 的地址偏移，需要条件翻转 bit6
      n0: 假设初始 src_addr 的 b6=0
      n1: src_addr ^= 0b1000000 后, b6=1
*/
...
__device__ __forceinline__ void ldmatrix_b(const half* src,
  ...
  // 按 half 2B 元素索引，由于写 share memory 是按照 float4=16B 粒度，因此 mask = 0b111>>(5+3) = 0b111>>8 =
  // 0b11100000000
...
__global__ void kernel_5(half* A, half* B, half* C, half* D, const float alpha, const float beta, const unsigned int M,
  ...
  /*
    <Hard Code to get the tileMemcpyLoad parameters>
    A tile:
    TILE_ROWS = BM_dim = 256
    TILE_COLS = BK_dim = 32
    TILE_COLS_VECTORIZED = TILE_COLS/8 = 4（float4 粒度）
    ROW_STEP = NUM_THREADS/TILE_COLS_VECTORIZED = 256/4 = 64
    NUM_ITERS = TILE_ROWS/ROW_STEP = 256/64 = 4

    B tile:
    TILE_ROWS = BK_dim = 32
    TILE_COLS = BN_dim = 256
    TILE_COLS_VECTORIZED = TILE_COLS/8 = 256/8 = 32（float4 粒度）
    ROW_STEP = NUM_THREADS/TILE_COLS_VECTORIZED = 256/32 = 8
    NUM_ITERS = TILE_ROWS/ROW_STEP = 32/8 = 4
  */
  float4 A_gmem_cache_reg[4];  // tileMemcpyLoad 每个线程缓存 4 个 float4
  float4 B_gmem_cache_reg[4];  // tileMemcpyLoad 每个线程缓存 4 个 float4
...
#pragma unroll
  ...
  for (unsigned int mma_m = 0; mma_m < mma_tiles_per_warp_m; mma_m++) {
    for (unsigned int mma_n = 0; mma_n < mma_tiles_per_warp_n; mma_n++) {
      ...
      // store D from register to global memory
...
void kernel_5_launch(sgemm_params device_sgemm_params, KernelLogger& timer, const unsigned int num_runs = 10) {
  ...
  for (int i = 0; i < num_runs; i++) {
    timer.Start();
    kernel_5<BM_dim, BN_dim, BK_dim, WM_dim, WN_dim, WK_dim, NumThreads><<<gridDim, blockDim, shmem_bytes>>>(
