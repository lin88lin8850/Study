...
void cublas_launch(sgemm_params device_sgemm_params, KernelLogger& timer, const unsigned int num_runs = 10) {
  ...
  cublasStatus_t status = cublasHgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha, device_sgemm_params.B, N,
                                      device_sgemm_params.A, K, &beta, device_sgemm_params.D, N);

  for (int i = 0; i < num_runs; i++) {
    timer.Start();
    status = cublasHgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha, device_sgemm_params.B, N,
                         device_sgemm_params.A, K, &beta, device_sgemm_params.C, N);
    timer.Stop();
