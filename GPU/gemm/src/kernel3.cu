...
__global__ void kernel_3(half* A, half* B, half* C, half* D, const float alpha, const float beta, const unsigned int M,
  ...
  // 按照 float4 16B 对齐: BK_dim / 8 == (BK_dim * sizeof(half)) / 16
  constexpr unsigned int SWIZZLE_BITS_A = int_log2(BK_dim / 8);
  constexpr unsigned int SWIZZLE_BITS_B = int_log2(BN_dim / 8);
  // tileMemcpySwizzle 从 global memeory 读取数据时，按照 float4 寻址，SWIZZLE_MASK = 0b111 << SWIZZLE_BITS
  // ldmatrix 从 shared memory 读取数据时，按照 byte 寻址，由于 float4=16B，所以需要多左移 4 位
  constexpr unsigned int SWIZZLE_MASK_A = 0b111 << (SWIZZLE_BITS_A + 4);
  constexpr unsigned int SWIZZLE_MASK_B = 0b111 << (SWIZZLE_BITS_B + 4);
...
void kernel_3_launch(sgemm_params device_sgemm_params, KernelLogger& timer, const unsigned int num_runs = 10) {
  ...
  for (int i = 0; i < num_runs; i++) {
    timer.Start();
    kernel_3<BM_dim, BN_dim, BK_dim, WM_dim, WN_dim, WK_dim, NumThreads><<<gridDim, blockDim, shmem_bytes>>>(
