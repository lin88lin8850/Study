...
__global__ void kernel_1(half* A, half* B, half* C, half* D, const float alpha, const float beta, const unsigned int M,
  ...
  /*
    ...
    4). A100 寄存器宽度是 32-bit，换算后需要 [MMA_M_dim][MMA_N_dim][2] 的 uint32_t 寄存器数组
  */
  ...
  /*
    1). MMA_M_dim=16, 使用 ldmatrix.sync.aligned.m8n8.x2.shared.b16 每次从 shared memory 读取 2 个 8x8 的 tile 到寄存器
    2). MMA_N_dim=8, 使用 ldmatrix.sync.aligned.m8n8.x1.trans.shared.b16 每次从 shared memory 读取 1 个 8x8 的 tile
    转置后存到寄存器
  */
        ...
        for (unsigned int mma_n = 0; mma_n < mma_tiles_per_warp_n; mma_n++) {
          for (unsigned int mma_m = 0; mma_m < mma_tiles_per_warp_m; mma_m++) {
            asm volatile(
                ...
                : "r"(A_register[mma_m][mma_k][0]), "r"(A_register[mma_m][mma_k][1]), "r"(B_register[mma_k][mma_n]),
                  "r"(acc_register[mma_m][mma_n][0]), "r"(acc_register[mma_m][mma_n][1]));
          }
        }
      }
  ...
  // 计算 alpha * AB + beta * C，并将结果存储回全局内存 D
...
void kernel_1_launch(sgemm_params device_sgemm_params, KernelLogger& timer, const unsigned int num_runs = 10) {
  ...
  // row-major 情况下，threadIdx.x 变化最快，适合用来表示“行内列号”，threadIdx.y 表示第几行
  ...
  for (int i = 0; i < num_runs; i++) {
    timer.Start();
    kernel_1<BM_dim, BN_dim, BK_dim, WM_dim, WN_dim, WK_dim, NumThreads><<<gridDim, blockDim, shmem_bytes>>>(
