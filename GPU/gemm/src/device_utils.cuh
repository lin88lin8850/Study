...
// writes
__device__ __forceinline__ void tileMemcpy(half* src, half* dst,
...
template <unsigned int TILE_ROWS, unsigned int TILE_COLS,
          unsigned int NUM_THREADS>
__device__ __forceinline__ void tileMemcpyUnrolled(
    half* src, half* dst, const unsigned int src_stride) {
  // # of threads is multiple of # of columns in the tile
...
#pragma unroll
  for (unsigned int i = 0; i < NUM_ITERS; i++) {
    dst[thread_row * TILE_COLS + thread_col] =
        src[thread_row * src_stride + thread_col];
    thread_row += ROW_STEP;
  }
}
...
template <unsigned int TILE_ROWS, unsigned int TILE_COLS,
          unsigned int NUM_THREADS, unsigned int SWIZZLE_BITS>
...
#pragma unroll
  for (unsigned int i = 0; i < NUM_ITERS; i++) {
    ...
    const unsigned int src_index =
        thread_row * src_stride_vectorized + thread_col;
...
template <unsigned int TILE_ROWS, unsigned int TILE_COLS,
          unsigned int NUM_THREADS, unsigned int ELEMENTS_PER_THREAD>
__device__ __forceinline__ void tileMemcpyLoad(
    half* src, float4 (&dst_reg)[ELEMENTS_PER_THREAD],
    const unsigned int src_stride) {
  // reinterpret input/output as float4
  float4* src_float4 = reinterpret_cast<float4*>(src);
  const unsigned int src_stride_vectorized = src_stride / 8;
...
__device__ __forceinline__ void tileMemcpySwizzleStore(
    float4 src_reg[ELEMENTS_PER_THREAD], half* dst) {
  constexpr unsigned int SWIZZLE_MASK = 0b111 << SWIZZLE_BITS;
...
__device__ __forceinline__ void tileMemcpySwizzleStoreA(
    const float4 (&src_reg)[ELEMENTS_PER_THREAD], half* dst) {
  constexpr unsigned int SWIZZLE_MASK_1 = 0b10000;
  constexpr unsigned int SWIZZLE_BITS_1 = 4;
  constexpr unsigned int SWIZZLE_MASK_2 = 0b1100;
  constexpr unsigned int SWIZZLE_BITS_2 = 2;
...
__device__ __forceinline__ void ldmatrix_m16n8(
    half* shmem, half (&reg)[4], unsigned int shmem_stride_bytes) {
...
__device__ __forceinline__ void ldmatrix_n8k8(half* shmem, half (&reg)[2],
...
__device__ __forceinline__ void mma_sync_m16n8k8(half (&D)[4], half (&A)[4],
                                                 half (&B)[2], half (&C)[4]) {
...
__device__ __forceinline__ void stmatrix_m16n8(half* dst, half (&reg)[4],
...
__device__ __forceinline__ void ldmatrix_m16n8_gmem(
    half* src, half (&reg)[4], unsigned int src_stride_bytes) {
  const unsigned int laneIdx = threadIdx.x % 32;
  uint32_t (&reg_)[2] = reinterpret_cast<uint32_t (&)[2]>(reg);
...
template <unsigned int dst_stride_bytes, unsigned int mma_tiles_per_warp_m,
          unsigned int mma_tiles_per_warp_n, unsigned int MMA_M_dim,
          unsigned int MMA_N_dim>
__device__ __forceinline__ void stmatrix_m16n8_swizzle(
    half (&src)[mma_tiles_per_warp_m][mma_tiles_per_warp_n][4], half* dst) {
  ...
  uint32_t (&src_)[mma_tiles_per_warp_m][mma_tiles_per_warp_n][2] =
      reinterpret_cast<
          uint32_t (&)[mma_tiles_per_warp_m][mma_tiles_per_warp_n][2]>(src);
...
#pragma unroll
    for (unsigned int mma_n = 0; mma_n < mma_tiles_per_warp_n; mma_n++) {
      ...
      // by BN block tile
      const unsigned int mma_tile_offset =
          ((mma_m * MMA_M_dim * dst_stride_int32) +
           (mma_n * mma_tile_stride_int32));
      const unsigned int thread_offset =
          mma_tile_offset + (thread_row * dst_stride_int32) + thread_col;
      const unsigned int swizzled_thread_offset =
          thread_offset ^ ((thread_offset & 0b11100000) >> 3);
      dst_[swizzled_thread_offset] = src_[mma_m][mma_n][0];
      dst_[swizzled_thread_offset + 8 * dst_stride_int32] =
          src_[mma_m][mma_n][1];
    }
  }
}
